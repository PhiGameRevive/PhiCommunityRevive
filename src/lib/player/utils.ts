...
import { clamp, getLines, isPec } from '$lib/utils';
import PhiEditerConverter from '../converters/phiediter';
import { convertPhiraChart } from '../converters/phiraChart';
...
export const loadChart = async (url: string, name: string = 'chart'): Promise<RpeJson | null> => {
  ...
  try {
    ...
    // 尝试 Phira chart（formatVersion 3，无 META.RPEVersion）
    try {
      const parsed = JSON.parse(text);
      if (parsed && parsed.formatVersion !== undefined && Array.isArray(parsed.judgeLineList)) {
        return convertPhiraChart(parsed);
      }
    } catch {
      /* 非 Phira JSON */
    }
...
export const getAudio = async (url: string): Promise<string> => {
  // 视频文件作为音乐（mp4/webm 等）：不下载 blob，直接返回原 URL，由 HTML5 Audio / Video 流式播放
  // （WebAudio decodeAudioData 无法解码视频容器；整文件下载到内存也会导致大文件谱面卡顿/崩溃）
  ...
  // 浏览器无法直接播放的音频格式（如未转码的格式）不进行转码，直接返回原始数据
  return URL.createObjectURL(originalAudio);
};
