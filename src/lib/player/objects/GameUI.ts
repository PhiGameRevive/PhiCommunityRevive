...
export class GameUI {
  ...
  private _fontSizes: number[] = [0, 63, 22, 46, 22, 34, 34, 24];
  ...
  constructor(scene: Game) {
    ...
    if (showAccuracy) {
      this._accuracy = this.createComponent(
        ...
        0,
        12,
        `${stats.displayStdDev.toFixed(3)} ms 路 ${stats.accuracy.toLocaleString(undefined, {
          style: 'percent',
  ...
  update() {
    ...
    this._accuracy?.setText(
      `${this._scene.statistics.displayStdDev.toFixed(3)} ms 路 ${this._scene.statistics.accuracy.toLocaleString(
        undefined,
        {
          style: 'percent',
          minimumFractionDigits: 2,
    ...
    [
      ...
      if ('resize' in obj) {
        obj.resize(this._scene.p(0.325));
      } else {
