...
export class Video extends GameObjects.Container {
  ...
  constructor(
    ...
    this._scene = scene;
    this._data = data;
    // path 为完整 URL（http/blob/data）时直接流式加载，不依赖 Phaser load.video 预加载
    // noAudio=true：背景视频静音播放（谱面音乐由独立音频轨提供，避免视频音轨叠加）
    if (/^(https?:|blob:|data:)/.test(data.path)) {
      this._video = new GameObjects.Video(scene, 0, 0);
      this._video.loadURL(data.path, true, 'anonymous');
    } else {
    ...
    this._video.on('metadata', () => {
      // PRPR 璋遍潰鐨?videos[].time 鍙己鐪侊紝瑙嗕负 0
      this._data.startTimeSec = getTimeSec(scene.bpmList, toBeats(this._data.time ?? [0, 0, 0]));
